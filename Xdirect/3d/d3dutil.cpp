...
RCSID( "$Id: d3dutil.cpp,v 1.1.1.1 2003/08/19 17:44:48 bernie Exp $" )
...
//-----------------------------------------------------------------------------
void D3DUtil_InitLight( D3DLIGHT *light, D3DLIGHTTYPE ltType, FLOAT x, FLOAT y, FLOAT z ) {
...
DWORD D3DUtil_GetDisplayDepth( LPDIRECTDRAW4 pDD4 ) {
	...
	if( pDD4 == NULL ) {
		...
		LPDIRECTDRAW pDD1;
		if( !pDirectDrawCreate || FAILED( pDirectDrawCreate( NULL, &pDD1, NULL ) ) )
			return 0L;
...
HRESULT _DbgOut( TCHAR *strFile, DWORD dwLine, HRESULT hr, TCHAR *strMsg ) {
	...
	TCHAR buffer[512+1];
	snprintf( buffer, 512, "%s(%ld): ", strFile, dwLine );
	OutputDebugString( buffer );
	OutputDebugString( strMsg );

	if( hr ) {
		snprintf( buffer, 512, "(hr=%08lx)\n", hr );
		OutputDebugString( buffer );
	}
